...
- Claude models that require [adaptive thinking](https://docs.aws.amazon.com/bedrock/latest/userguide/claude-messages-adaptive-thinking.html) get `output_config.effort` with the value the Anthropic provider sends.
- [gpt-oss](https://docs.aws.amazon.com/bedrock/latest/userguide/model-parameters-openai.html) gets `reasoning_effort`.
- [Nova 2](https://docs.aws.amazon.com/nova/latest/nova2-userguide/extended-thinking.html) gets `reasoningConfig`.
- At `:high` on Nova 2, the default `max_tokens` ReqLLM adds is left out; a `max_tokens` you set is sent.
